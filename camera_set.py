...
class CameraSet:
    ...
    def __init__(self):
        ...
        self._curr_cam = self._cams[self._curr_key]

    def add_cam(self, cam_label=None, new_cam=FlyCamera(fov=60)):
        if issubclass(type(new_cam), BaseCamera):
            if not cam_label:
                cam_label = "cam_" + str(len(self._cams))
            self._cams.update({cam_label: new_cam})
    ...
    def cam_state(self, cam_key=None):
        ...
        cam_states = self._cams[cam_key].get_state()
    ...
    @property
    def cam_count(self):
        return len(self._cams.keys())

    @property
    def curr_cam(self):
        return self._cams[self._curr_key]
